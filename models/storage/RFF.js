...
function RFF (bytes) {
    ...
    const encrypt = (bytes, options) => decrypt(bytes, options);

    // util
    const toUnixTime = d => d.valueOf() / 1000 - new Date().getTimezoneOffset() * 60;
    ...
    const fileHeadersBytes = decrypt(reader.bytes.slice(this.Offset, this.Offset + this.Files.length * fileHeaderSize), {
    ...
    for (let i = 0; i < this.Files.length; i++) {
        ...
        this.Files[i].bytes = (this.Files[i].flags & 16) ? decrypt(bytes, { seed: 0, offset: 0, limit: 256 }) : bytes;
    }
    ...
    this.Serialize = () => {
        ...
        for (let i = 0; i < this.Files.length; i++) {
            ...
            this.Files[i].bytes = (this.Files[i].flags & 16) ? encrypt(this.Files[i].bytes, { seed: 0, offset: 0, limit: 256 }) : this.Files[i].bytes;
        ...
        // create byte writer
        const writer = new ByteWriter(headerSize + this.Files.reduce((sum, f) => sum += f.size , 0) + this.Files.length * fileHeaderSize);
        ...
        writer.int32(headerSize + this.Files.reduce((sum, f) => sum += f.size , 0));
        ...
        const fileHeaderWriter = new ByteWriter(this.Files.length * fileHeaderSize);
        ...
        for (let i = 0; i < this.Files.length; i++) {
            ...
            fileHeaderWriter.int32(this.Files[i].time || toUnixTime(new Date())); // last modified
        ...
        writer.write(encrypt(fileHeaderWriter.bytes, {
            seed: offset & 0xFF,
            offset: 0,
            limit: 0
        }));
